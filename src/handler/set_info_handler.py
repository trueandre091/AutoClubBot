...
async def set_info_name_handler(update: Update, context: CallbackContext):
    logger.info("Set name %s %s: %s", update.message.from_user.id, update.message.from_user.username, update.message.text)

    message = update.message.text
    if not message == set_info_view["buttons"]["5"]:
...
async def set_info_car_brand_handler(update: Update, context: CallbackContext):
    logger.info("Set car brand %s %s: %s", update.message.from_user.id, update.message.from_user.username, update.message.text)
...
async def set_info_car_drive_handler(update: Update, context: CallbackContext):
    logger.info("Set car drive %s %s: %s", update.message.from_user.id, update.message.from_user.username, update.message.text)
...
async def set_info_car_power_handler(update: Update, context: CallbackContext):
    logger.info("Set car power %s %s: %s", update.message.from_user.id, update.message.from_user.username, update.message.text)
...
async def set_info_car_number_handler(update: Update, context: CallbackContext):
    logger.info("Set car number %s %s: %s", update.message.from_user.id, update.message.from_user.username, update.message.text)
    ...
    await chat.send_message(set_info_view["8"], reply_markup=ReplyKeyboardRemove())
    await sleep(1)
...
async def cancel_set_info(update: Update, context: CallbackContext) -> int:
    logger.info("Cancel set info %s %s: %s", update.message.from_user.id, update.message.from_user.username, update.message.text)
